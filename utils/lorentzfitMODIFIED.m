% function varargout = lorentzfitMODIFIED(x,y,varargin)
function outputData = lorentzfitMODIFIED(x,y,varargin)
%
% Function modified by R Bettles to return standard error as well
%
...
optargs = {[],[],'3c',optimset('display','none','TolFun',max(mean(y(:))*1e-6,1e-15),'TolX',max(mean(x(:))*1e-6,1e-15))};
...
switch lower(nparams)
    ...
    case '3'
        ...
        [params,resnorm,residual,exitflag,output,lambda,J] = lsqcurvefit(@lfun3,p0,x,y,lb,ub,options);
        yprime = lfun3(params,x);
        
        errorParams = nlparci(params,residual,'jacobian',J);
    case '3c'
        % Define P0, LB, UB
        if isempty(p0);
            p0 = [p1 p2 p3 c];
...
%         [params,resnorm,residual] = lsqcurvefit(@lfun3c,p0,x,y,lb,ub,options);
        [params,resnorm,residual,exitflag,output,lambda,J] = lsqcurvefit(@lfun3c,p0,x,y,lb,ub,options);
...
outputData = {yprime,params,resnorm,residual,errorParams};
